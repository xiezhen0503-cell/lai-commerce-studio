...
const exportOptions:Record<string,Array<[string,string]>>={
  proposal:[["Markdown","md"],["Word","docx"],["HTML","html"],["TXT","txt"],["JSON","json"]],script:[["Markdown","md"],["Word","docx"],["HTML","html"],["TXT","txt"],["JSON","json"]],"image-prompt":[["Markdown","md"],["Word","docx"],["HTML","html"],["TXT","txt"],["JSON","json"]],caption:[["Markdown","md"],["Word","docx"],["HTML","html"],["TXT","txt"],["JSON","json"]],report:[["Markdown","md"],["Word","docx"],["HTML","html"],["TXT","txt"],["JSON","json"]],prompt:[["Markdown","md"],["Word","docx"],["HTML","html"],["TXT","txt"],["JSON","json"]],storyboard:[["JSON","json"],["Excel","xlsx"],["CSV","csv"],["Word","docx"]],"video-storyboard":[["JSON","json"],["Excel","xlsx"],["CSV","csv"],["Word","docx"]],schedule:[["JSON","json"],["Excel","xlsx"],["CSV","csv"],["Word","docx"]],handoff:[["JSON","json"],["Excel","xlsx"],["CSV","csv"],["Word","docx"]],image:[["原图","original"],["生成记录","json"]],video:[["项目包 ZIP","zip"],["离线预览","html"],["配置 JSON","json"]]
};
  ...
  function ArtifactView(){
    ...
    if(selected.type==="image"&&!editing){
      try{
        ...
        if(image.assetUri){const real=Boolean(image.metadata?.externalGeneration);const warnings=Array.isArray(image.production?.warnings)?image.production.warnings:[];return <div className="card-pad"><img src={image.assetUri} alt={selected.title} style={{display:"block",width:"100%",maxWidth:680,margin:"0 auto",borderRadius:18,boxShadow:"var(--shadow)"}}/><div className="grid-2" style={{marginTop:16}}><Brief label="图片来源" value={real?`${image.metadata?.service||"外部图片模型"} · ${image.metadata?.model||""}`:"确定性 SVG 构图预览"}/><Brief label="商品照片参考" value={image.production?.referenceSourceId?"已使用上传照片":"未使用商品照片"}/></div>{warnings.map((warning:string)=><p className="hint" key={warning} style={{marginTop:8}}>{warning}</p>)}</div>};
