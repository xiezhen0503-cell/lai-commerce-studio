...
export function ProjectWorkspace({ initial }: { initial: any }) {
    ...
    if(selected.type==="image"&&!editing){
      try{
        ...
        if(image.assetUri){
          ...
          const legacyRealImage=real&&!['image-v2-font-ocr','image-v3-font-ocr-advisory'].includes(String(image.metadata?.imagePipelineVersion||''));
          if(legacyRealImage)return <div className="card-pad"><img src={image.assetUri} alt={`${selected.title}旧版结果`} style={{display:"block",width:"100%",maxWidth:680,margin:"0 auto",borderRadius:18,boxShadow:"var(--shadow)"}}/><div className="card" style={{padding:16,marginTop:16,background:"#fff7ed",border:"1px solid #fdba74"}}><div className="eyebrow">旧版图片提示</div><p className="subtitle">这张历史图片没有使用新版内置中文字体和辅助 OCR。系统不再拦截展示，请你人工查看是否需要重新生成。</p><Link className="button primary" href="/" style={{marginTop:12}}>返回 AI 工作台重新生成</Link></div></div>;
          return <div className="card-pad"><img src={image.assetUri} alt={selected.title} style={{display:"block",width:"100%",maxWidth:680,margin:"0 auto",borderRadius:18,boxShadow:"var(--shadow)"}}/><div className="grid-2" style={{marginTop:16}}><Brief label="图片来源" value={real?`${image.metadata?.service||"外部图片模型"} · ${image.metadata?.model||""}`:"确定性 SVG 构图预览"}/><Brief label="中文排版与辅助 OCR" value={real?`${image.metadata?.overlayFont||"Noto Sans SC"} · ${image.metadata?.typographyQa?.status==="passed"?"未发现明显模型文字":image.metadata?.typographyQa?.status==="visible-text-detected"?"发现文字痕迹，请人工查看":"未执行"}`:"确定性排版"}/><Brief label="商品照片参考" value={image.production?.referenceSourceId?"已使用上传照片":"未使用商品照片"}/></div>{warnings.map((warning:string)=><p className="hint" key={warning} style={{marginTop:8}}>{warning}</p>)}</div>;
        }
      }catch{/* 非结构化历史版本继续按文本显示 */}
    }
    if(selected.type==="video"&&!editing)return <div className="card-pad"><div className="card" style={{padding:22,background:"var(--surface-soft)"}}><div className="eyebrow">Verified MP4 Render</div><h2>可播放、可下载的商品视频成片</h2><p className="subtitle">生成完成前服务器已经逐帧编码并检查 MP4 文件结构；播放器使用分段读取，下载得到同一份 H.264 文件。</p><video controls playsInline preload="metadata" src={`/api/v1/artifacts/${selected.id}/export?format=mp4&inline=1`} style={{width:"100%",maxHeight:620,background:"#111",borderRadius:16,marginTop:16}}/><div className="row" style={{marginTop:14,flexWrap:"wrap"}}><a download className="button primary" href={`/api/v1/artifacts/${selected.id}/export?format=mp4`}><Download size={14}/>下载 MP4</a><a download className="button secondary" href={`/api/v1/artifacts/${selected.id}/export?format=srt`}><Download size={14}/>字幕</a><a download className="button secondary" href={`/api/v1/artifacts/${selected.id}/export?format=png`}><Download size={14}/>封面</a></div></div><details style={{marginTop:14}}><summary className="small muted">查看渲染配置</summary><pre className="editor">{version?.content}</pre></details></div>;
  ...
  function MainPanel(){
    ...
    if(activeTab==="导出")return <div className="card"><div className="card-head"><div><h2>导出成果</h2><div className="list-meta">每个下载都对应当前版本与事实快照，不包含服务端密钥</div></div>{data.artifacts.length>0&&<a download className="button primary" href={`/api/v1/projects/${projectId}/export`}><Download size={14}/>下载全部 ZIP</a>}</div><div className="card-pad" style={{paddingTop:4,paddingBottom:4}}><ul className="list">{data.artifacts.map((artifact:any)=>{const options=exportOptions[artifact.type]||[["JSON","json"],["TXT","txt"]];return <li className="list-item" key={artifact.id}><div className="list-icon"><Download size={17}/></div><div className="list-copy"><div className="list-title">{artifact.title}</div><div className="list-meta">{options.map(([label])=>label).join(" / ")} · v{artifact.currentVersion} · {statusLabel(artifact.status)}</div></div><div className="row" style={{flexWrap:"wrap",justifyContent:"flex-end"}}>{options.map(([label,format],index)=><a download className={`button ${index===0?"primary":"secondary"}`} style={{minHeight:30,padding:"0 10px"}} key={format} href={`/api/v1/artifacts/${artifact.id}/export?format=${format}`}><Download size={12}/>{label}</a>)}</div></li>})}</ul></div></div>;
