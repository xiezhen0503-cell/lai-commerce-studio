...
export function ProjectWorkspace({ initial }: { initial: any }) {
  ...
  function ArtifactView(){
    ...
    if(selected.type==="image"&&!editing){
      try{
        ...
        if(image.assetUri)return <div className="card-pad"><img src={image.assetUri} alt={selected.title} style={{display:"block",width:"100%",maxWidth:680,margin:"0 auto",borderRadius:18,boxShadow:"var(--shadow)"}}/><div className="grid-2" style={{marginTop:16}}><Brief label="画面任务" value={image.storyboard?.composition||"确定性预览"}/><Brief label="文字图层" value={image.storyboard?.overlay||"发布前人工确认"}/></div><p className="hint" style={{marginTop:12}}>这是系统真实生成并保存的 SVG 构图预览；商品精修图仍需接入图片生成 Provider 或上传真实商品图。</p></div>;
      }catch{/* 非结构化历史版本继续按文本显示 */}
    }
    if(selected.type==="video"&&!editing)return <div className="card-pad"><div className="card" style={{padding:22,background:"var(--surface-soft)"}}><div className="eyebrow">Remotion Composition</div><h2>30 秒促销活动视频草稿</h2><p className="subtitle">浏览器会实时执行模板动画，并把商品名、规格、CTA 和安全区作为确定性图层渲染。</p><a className="button primary" href="/video-preview?template=Promo30" style={{marginTop:16}}><Play size={14}/>打开可播放预览</a></div><pre className="editor" style={{marginTop:14}}>{version?.content}</pre></div>;
  ...
  function MainPanel(){
    ...
    if(activeTab==="导出")return <div className="card"><div className="card-head"><div><h2>导出成果</h2><div className="list-meta">每个下载都对应一个版本化成果，不包含服务端密钥</div></div></div><div className="card-pad" style={{paddingTop:4,paddingBottom:4}}><ul className="list">{data.artifacts.map((artifact:any)=><li className="list-item" key={artifact.id}><div className="list-icon"><Download size={17}/></div><div className="list-copy"><div className="list-title">{artifact.title}</div><div className="list-meta">Markdown · v{artifact.currentVersion} · {statusLabel(artifact.status)}</div></div><a className="button secondary" href={`/api/v1/artifacts/${artifact.id}/export`}><Download size={13}/>下载</a></li>)}</ul></div></div>;
    return <div className="card"><div className="card-head"><div><h2>{selected?.title||"项目尚无生成成果"}</h2><div className="list-meta">{selected?`${statusLabel(selected.status)} · 版本 v${selected.currentVersion}`:"点击生成整套活动开始"}</div></div><div className="row"><button className="button secondary" onClick={bundle} disabled={loading==="bundle"}>{loading==="bundle"?<LoaderCircle size={14}/>:<Sparkles size={14}/>}生成整套活动</button>{selected&&<><button className="button secondary" onClick={()=>setEditing(true)}><Pencil size={14}/>编辑</button><button className="button primary" onClick={review} disabled={loading==="review"}><ShieldCheck size={14}/>提交审核</button></>}</div></div><ArtifactView/></div>;
  }
