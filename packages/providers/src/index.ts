...
export class MockTextProvider implements TextGenerationProvider {
  ...
  async generate(spec: PromptSpec, _prompt: string) {
    ...
    await wait(20);
    const proposal = `# ${spec.objective}\n\n## 方案摘要与经营判断\n首轮不追求铺量，先用具体生活场景验证目标人群是否愿意停留、理解并进一步了解商品。内容只讲有依据的商品信息，不把创意建议写成事实。目标平台为 ${spec.targetPlatforms.join("、")}，各平台使用同一事实底稿，但开场节奏和 CTA 分别适配。\n\n## 目标与指标\n- 业务目标：完成新品认知与首轮转化验证，7 天形成一轮可复盘样本。\n- 核心 KPI：点击率、3 秒留存、完播率、收藏/互动率和加购率；历史基线与预算均待确认。\n- 成功标准：至少找到一个在两次发布中表现稳定的内容角度；样本不足时不下确定性结论。\n\n## 目标人群与平台策略\n目标人群是忙碌、希望快速看懂商品信息的城市用户。购买阻力通常不是“不感兴趣”，而是看不清配料、规格、使用场景和价格条件。抖音用前 3 秒冲突与动作推进，小红书用清单、实拍细节和可收藏信息承接。\n\n## 核心策略\n1. 用早餐、午后和加班三个具体场景承接需求，不写泛泛生活方式口号。\n2. 每条内容只讲一个利益点，并附事实来源；没有依据的价格、日期和功效继续标记待确认。\n3. 先小样本测试开场和商品信息顺序，再决定是否增加制作与投放资源。\n\n## 内容矩阵\n- 认知主线：展示原料或包装细节；素材为实拍商品与背标；形式为 15 秒近景视频；CTA 为“先看清商品信息”。\n- 理解主线：解释规格、使用方式与适用场景；素材为步骤图和真实道具；形式为图文清单；CTA 为“收藏后对照选择”。\n- 转化主线：说明购买路径和已确认活动机制；素材为商品卡；价格与日期待确认前不上屏；CTA 使用品牌已批准版本。\n\n## 7 天执行排期\n- 第 1 天：运营确认目标、平台、事实卡和待确认项，交付一页任务简报。\n- 第 2 天：编导制作 3 个开场和 2 套封面，交付可拍摄脚本与素材清单。\n- 第 3 天：拍摄包装、场景和使用动作，按镜头表核对素材完整性。\n- 第 4 天：剪辑首版并完成字幕、事实和合规复核，输出两个 A/B 版本。\n- 第 5 天：在选定平台发布小样本，记录点击率、3 秒留存、完播率和互动率。\n- 第 6 天：复盘评论问题和漏斗数据，只修改一个关键变量后再测试。\n- 第 7 天：汇总结果，决定继续加码、调整内容主线或暂停，并保存结论与证据。\n\n## 资源与预算假设\n- 负责人：运营负责事实和排期，编导负责脚本，拍摄/剪辑负责成片；一人兼任时需明确交付时间。\n- 素材：真实商品、包装背标、使用道具、竖屏拍摄设备和品牌视觉文件。\n- 工时与预算：具体数字待确认；未确认预算前只做轻量自然流量测试，不承诺投放结果。\n\n## 数据复盘与止损\n第 5 天和第 7 天复盘。若 3 秒留存和点击率连续两次低于账号历史基线，则暂停同一开场并更换钩子；若互动好但加购弱，优先补商品信息和购买路径；若样本量不足，则延长观察，不把偶然波动包装成结论。只有核心指标稳定高于基线时才继续加码。\n\n## 事实依据、创意假设与发布前待确认\n- 已确认事实只能来自当前事实快照和本次资料，发布时保留来源。\n- 早餐、午后和加班场景属于创意假设，需要用真实素材验证。\n- 商品名称、规格、配料、活动价格、活动时间、产地、资质和功效逐项待确认；没有证据的字段不得发布。\n- 不使用治疗、减肥保证、绝对化表述或虚构用户评价。\n`;
